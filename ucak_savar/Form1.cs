...
namespace ucak_savar
{
    public partial class Form1 : Form
    {
        ...
        private void ucaklariolustur()
        {
            ...
            ucak.Size = new Size(50, 50); 
            ucak.SizeMode = PictureBoxSizeMode.StretchImage;
            ucak.ImageLocation = "png/ucak.png"; 
            ucak.Location = new Point(yer, 50);  
            ucak.Visible = true;
        ...
        private void ucaklaridusur()
        {
            for (int i = 0; i < ucaklar.Count; i++)
            {
                    ...
                    if (ucaklar[i].Location.Y >= 760 ||  (ucaklar[i].Bounds.IntersectsWith(ucaksavar.Bounds)))
                    {
                        oyunu_bitir();
                    }
             ...
             for (int i = 0; i < mermiler.Count; i++)
             {
                for (int j = 0; j < ucaklar.Count; j++)
                {
                    try
                    {
                        ...
                        if (mermiler[i].Bounds.IntersectsWith(ucaklar[j].Bounds))
                        {
                            puan += 5;
                            if (puan >= 50)
                            {
